...
describe('HttpClient', () => {
    ...
    describe('fetch', () => {
        ...
        it('should not retry on non-retryable status', async () => {
            const mockResponse = { ok: false, status: 400, statusText: 'Bad Request', json: vi.fn().mockResolvedValue({}) };
            global.fetch = vi.fn().mockResolvedValue(mockResponse);
            const { to, co } = await import('../../../extension/utils/shortcuts/global.js');
            to.mockReturnValue('timeoutId');
        ...
        it('should handle network errors', async () => {
            global.fetch = vi.fn().mockRejectedValue(Object.assign(new Error('Connection reset'), { code: 'ECONNRESET' }));
