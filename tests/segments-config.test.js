...
describe('Segment Configuration', () => {
  it('default config includes all segment categories', () => {
    ...
    expect(d.segments.enabled).toBe(true);
    ...
    expect(d.segments.categories.interaction).toEqual({ action: 'skip', speed: 2 });
    expect(d.segments.categories.intro).toEqual({ action: 'skip', speed: 2 });
    expect(d.segments.categories.outro).toEqual({ action: 'skip', speed: 2 });
    expect(d.segments.categories.preview).toEqual({ action: 'skip', speed: 2 });
    expect(d.segments.categories.music_offtopic).toEqual({ action: 'skip', speed: 2 });
